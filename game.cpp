...
void handle_main_menu_input(const Event& ev, RenderWindow& window) 
{
    ...
    if (ev.type == Event::MouseButtonPressed && ev.mouseButton.button == Mouse::Left) 
    {
        ...
        if (is_button_hovered(btn_new_game, mouse_pos))
        {
            reset_game();
        }
        else if (is_button_hovered(btn_settings, mouse_pos)) 
        {
            game_state = SETTINGS_MENU;
            selected_menu = -1; 
        }
        else if (is_button_hovered(btn_leaderboard, mouse_pos))
        {
            game_state = LEADERBOARD_MENU;
            selected_menu = -1;
        }
        else if (is_button_hovered(btn_exit, mouse_pos))
        {
            window.close();
        }
    }
...
void handle_settings_input(const Event& ev, RenderWindow& window) 
{
    ...
    {
        Vector2f mouse_pos = window.mapPixelToCoords(Mouse::getPosition(window));
        if (is_button_hovered(btn_difficulty, mouse_pos)) 
        {
            game_state = DIFFICULTY_MENU;
            selected_menu = -1; 
        }
        ...
        else if (is_button_hovered(btn_back_settings, mouse_pos)) 
        {
            game_state = MAIN_MENU; 
            selected_menu = -1; 
        }
...
void handle_difficulty_input(const Event& ev, RenderWindow& window) 
{
    ...
    if (ev.type == Event::MouseButtonPressed && ev.mouseButton.button == Mouse::Left)
    {
        ...
        else if (is_button_hovered(btn_back_difficulty, mouse_pos)) 
        {
            game_state = SETTINGS_MENU;
            selected_menu = -1; 
        }
...
void handle_pause_input(const Event& ev, RenderWindow& window) 
{
    ...
    {
        Vector2f mouse_pos = window.mapPixelToCoords(Mouse::getPosition(window));
        if (is_button_hovered(btn_resume, mouse_pos)) 
        {
            game_state = PLAYING;
            bg_music.play(); 
            selected_menu = -1;
        }
        else if (is_button_hovered(btn_restart, mouse_pos))
        {
            reset_game();
        }
        else if (is_button_hovered(btn_main_menu, mouse_pos)) 
        {
            game_state = MAIN_MENU; 
            selected_menu = -1; 
        }
        else if (is_button_hovered(btn_exit_pause, mouse_pos))
        {
            window.close();
        }
    }
...
void handle_game_over_input(const Event& ev, RenderWindow& window) 
{
    ...
    {
        Vector2f mouse_pos = window.mapPixelToCoords(Mouse::getPosition(window));
        if (is_button_hovered(btn_play_again, mouse_pos))
        {
            reset_game();
        }
        else if (is_button_hovered(btn_main_over, mouse_pos)) 
        {
            game_state = MAIN_MENU; 
            selected_menu = -1; 
        }
        else if (is_button_hovered(btn_exit_over, mouse_pos))
        {
            window.close();
        }
    }
...
void draw_game_over(RenderWindow& window) 
{
    ...
    final_score_text.setPosition((width - score_bounds.width) / 2, 40 + bounds.height + 20);
